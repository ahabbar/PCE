    ...
    with st.expander("Actions", expanded=False):
        ...
        with col_exit:
            ...
            if exit_key not in st.session_state:
                ...
                st.success("Exit plan generated by Agent 6. Review, then confirm discharge.")
                ...
                if cc_confirm.button("✓ Confirm & Discharge", key=f"confirm_exit_{patient_id}",
                    ...
                    try:
                        cr = requests.post(
                            f"{API_URL}/queue/{patient_id}/discharge",
                            json={"disposition": disp_for_confirm,
                                  "confirmed_diagnosis": diag_for_confirm,
                                  "doctor_name": pt.get("assigned_doctor", "")},
                            timeout=10,
                        )
                        if cr.status_code == 200:
                            st.session_state.pop(exit_key, None)
                            st.success(f"Discharged ({disp_for_confirm})")
                            st.rerun(scope="app")
                        else:
